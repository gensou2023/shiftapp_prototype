...
### iOS Safari 対応チェックリスト

- [ ] `html { overflow: hidden; width: 100%; }` が設定されているか
- [ ] CSS Grid で `1fr` を使っている箇所に固定幅の子要素がないか
- [ ] `<input type="date">` に `-webkit-appearance: none` が適用されているか
- [ ] `100dvh` を使用し、`100vh` をフォールバックとして残しているか
- [ ] 動的に追加される要素（シフト生成結果等）のレイアウトも確認したか

### デバッグ手順

1. スクリーンショットで「操作前」と「操作後」の状態を切り分ける
2. どの操作（ボタン押下、データ追加等）で崩れるかを特定する
3. 根本原因を特定してから修正に着手する（推測パッチを重ねない）
